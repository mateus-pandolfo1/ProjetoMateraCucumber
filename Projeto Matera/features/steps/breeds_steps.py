...
@when('I send a GET request to "/breeds" with an invalid limit "{limit_value}"')
def step_send_get_request_with_invalid_limit(context, limit_value):
    context.response = requests.get(BASE_URL + "/breed", params={"limit": limit_value})
